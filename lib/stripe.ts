...
export class StripeService {
  ...
  static async createCheckoutSession(
    ...
    return await stripe.checkout.sessions.create({
      ...
      subscription_data: { metadata: { source: 'careerpath_app' } }
    })
  }

  // ✅ Use this when you don't have pre-created Stripe price IDs
  static async createCheckoutSessionInline(
    customerId: string,
    plan: { amount: number; interval: 'month' | 'year'; intervalCount?: number; name: string },
    successUrl: string,
    cancelUrl: string
  ) {
    return await stripe.checkout.sessions.create({
      ...
      line_items: [
        {
          price_data: {
            ...
            product_data: {
              name: plan.name,
              description: 'Career Advisor Platform Subscription',
            },
  ...
  static async getUsage(subscriptionItemId: string) {
    // Get usage record summaries for metered billing if needed
