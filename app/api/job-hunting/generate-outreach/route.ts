...
Thanks for your time!`,
    ...
    // Build content string — guard against null to satisfy NOT NULL constraint
    let draftContent: string
    if (type === 'email') {
      draftContent = outreachData?.subject && outreachData?.content
        ? `Subject: ${outreachData.subject}\n\n${outreachData.content}`
        : `Subject: Application for ${jobData?.title || 'Open Position'}\n\nDear Hiring Manager,\n\nI am writing to express my interest in the ${jobData?.title || 'position'} at ${jobData?.company || 'your company'}.\n\nBest regards,\n[Your Name]`
    } else {
      draftContent = outreachData?.content || `Dear Hiring Manager,\n\nI'm interested in the ${jobData?.title || 'position'} at ${jobData?.company || 'your company'}.\n\nBest regards,\n[Your Name]`
    }
