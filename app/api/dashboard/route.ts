  ...
  try {
    ...
    const stats = {
      ...
      roadmapProgress: userSkills.length > 0
        ? Math.round(userSkills.reduce((acc: number, skill: any) => acc + (skill.current_level || 0), 0) / userSkills.length)
        : 0,
    ...
    const skillProgress = userSkills.slice(0, 6).map((skill: any) => ({
...
// Skip Groq API entirely for dashboard — it rate-limits and returns malformed JSON.
// Rule-based recommendations are instant, reliable and personalized.
function generateAIRecommendations(skills: any[], colleges: any[], quizResults: any[], activities: any[]) {
