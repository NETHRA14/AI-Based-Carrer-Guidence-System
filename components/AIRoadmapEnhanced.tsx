...
interface CareerRoadmap {
  ...
  description: string
  career_goal: string
  careerGoal: string         // alias expected by RoadmapVisualization (always set = career_goal)
...
export default function AIRoadmapEnhanced() {
  ...
  // If ?career= query param is present (from quiz result), pre-fill and open generator
  useEffect(() => {
    const careerParam = searchParams.get('career')
    if (careerParam) {
      setFormData(prev => ({ ...prev, careerGoal: decodeURIComponent(careerParam) }))
                ...
                <div className="space-y-4 max-h-[600px] overflow-y-auto scrollbar-thin scrollbar-thumb-neon-cyan/20">
                  {roadmaps.map((roadmap, index) => (
                    <motion.div
                      ...
                      className={`group bg-gradient-to-br from-black/40 to-purple-900/20 backdrop-blur-sm rounded-xl border p-5 cursor-pointer transition-all duration-300 ${selectedRoadmap?.id === roadmap.id
            ...
            {selectedRoadmap ? (
              <div className={`${isFullscreen ? 'fixed inset-0 z-50 bg-space-dark' : ''}`}>
                <div className={`${isFullscreen
                  ...
                  >
                    <div className="absolute inset-0 bg-gradient-to-br from-black/20 via-transparent to-purple-900/20 pointer-events-none rounded-xl" />
                        ...
                        {['beginner', 'intermediate', 'advanced'].map((level) => (
                          <label key={level} className="flex items-center cursor-pointer group">
                            <input
                              ...
                              className="sr-only"
                            />
                            <div className={`w-4 h-4 rounded-full border-2 mr-3 transition-all ${formData.currentLevel === level
                                ? 'border-neon-cyan bg-neon-cyan'
                                : 'border-gray-500 group-hover:border-neon-cyan/50'
                              }`} />
                            <span className={`capitalize transition-colors ${formData.currentLevel === level ? 'text-neon-cyan' : 'text-gray-300 group-hover:text-white'
                              }`}>
                              {level}
                            </span>
                          </label>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                      {interestOptions.map((interest) => (
                        <motion.button
                          ...
                          className={`p-3 rounded-xl border text-sm font-medium transition-all ${formData.interests.includes(interest)
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                      {skillOptions.map((skill) => (
                        <motion.button
                          ...
                          className={`p-3 rounded-xl border text-sm font-medium transition-all ${formData.currentSkills.includes(skill)
