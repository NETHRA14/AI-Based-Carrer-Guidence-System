...
export default function RoadmapVisualization({ roadmap }: RoadmapVisualizationProps) {
      ...
      {phases.map((phase, phaseIndex) => (
        <div key={phase.id} className="relative">
          ...
          <div className="flex items-start space-x-4 mb-4">
            <div className={`w-12 h-12 rounded-full flex items-center justify-center border-2 ${phase.completed
          ...
          <div className="ml-16 space-y-3">
            {phase.milestones?.map((milestone, milestoneIndex) => (
              <motion.div
                ...
                className={`bg-black/30 backdrop-blur-sm border rounded-lg p-4 cursor-pointer hover:border-neon-cyan/50 transition-colors ${selectedNode?.id === milestone.id ? 'border-neon-cyan' : 'border-white/10'
                  }`}
  ...
  const renderNetworkView = () => {
    ...
    // Compute canvas size from actual node positions
    const maxX = displayNodes.reduce((m, n) => Math.max(m, n.position.x + NODE_W + PADDING), 600)
    const maxY = displayNodes.reduce((m, n) => Math.max(m, n.position.y + NODE_H + PADDING), 400)

    return (
      <div className="w-full overflow-auto rounded-lg bg-black/20" style={{ maxHeight: '70vh' }}>
        <div className="relative" style={{ width: maxX, minHeight: maxY }}>
          ...
          >
            ...
            {displayConnections.map((connection, index) => {
              const src = displayNodes.find(n => n.id === connection.source)
              const tgt = displayNodes.find(n => n.id === connection.target)
          ...
          {displayNodes.map((node, index) => (
            <motion.div
              ...
              className={`absolute rounded-xl border-2 flex flex-col items-center justify-center px-3 py-2 cursor-pointer text-center transition-shadow ${node.type === 'phase'
                ? 'bg-neon-cyan/10 border-neon-cyan text-neon-cyan shadow-lg shadow-neon-cyan/10'
                : 'bg-purple-500/10 border-purple-400 text-purple-300 shadow-lg shadow-purple-500/10'
                } ${selectedNode?.id === node.id ? 'ring-2 ring-white/60 scale-105' : ''}`}
              style={{
                left: node.position.x,
                top: node.position.y,
                width: NODE_W,
                minHeight: NODE_H,
                zIndex: selectedNode?.id === node.id ? 20 : 1,
              }}
              onClick={() => setSelectedNode({ id: node.id, type: node.type, position: node.position, data: node.data })}
            >
              <span className={`text-xs font-bold leading-tight ${node.type === 'phase' ? 'uppercase tracking-wide' : ''}`}>
                {node.data.title || 'Node'}
              </span>
              {node.data.duration && (
                <span className="text-[10px] opacity-60 mt-0.5">{node.data.duration}</span>
              )}
            </motion.div>
          ))}
        </div>
        ...
        {displayNodes.length === 0 && (
          <div className="flex items-center justify-center h-48 text-gray-500 text-sm">
            No network data available for this roadmap.
          </div>
        )}
      </div>
    )
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center space-x-3">
              <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${phase.completed ? 'bg-green-500' : 'bg-neon-cyan'
                }`}>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-3">
            {phase.milestones?.map((milestone) => (
              <div
                key={milestone.id}
                className={`border rounded-lg p-3 cursor-pointer transition-colors hover:border-neon-cyan/50 ${milestone.completed ? 'border-green-500/50 bg-green-500/10' : 'border-white/10 bg-black/20'
                  }`}
                onClick={() => setSelectedNode({ id: milestone.id, type: 'milestone', position: { x: 0, y: 0 }, data: milestone })}
              >
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center space-x-2">
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${view === 'phases'
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${view === 'timeline'
              ? 'bg-neon-cyan text-black'
              : 'bg-black/30 text-gray-300 hover:text-white'
              }`}
          >
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${view === 'network'
              ? 'bg-neon-cyan text-black'
              : 'bg-black/30 text-gray-300 hover:text-white'
              }`}
          >
                      ...
                      {selectedNode.data.resources.slice(0, 3).map((resource: Resource, index: number) => (
                        <div key={index} className="bg-black/20 rounded-lg p-3">
                          <div className="flex items-center justify-between mb-1">
                            ...
                            <span className={`text-xs px-2 py-1 rounded ${resource.cost === 'Free' ? 'bg-green-500/20 text-green-400' : 'bg-orange-500/20 text-orange-400'
                              }`}>
                              {resource.cost}
                            </span>
                          </div>
