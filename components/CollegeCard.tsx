...
const CollegeCard: React.FC<CollegeCardProps> = ({ college, onSave, isSaved = false }) => {
  ...
  const getTypeColor = (type: string | null | undefined) => {
    switch ((type || '').toLowerCase()) {
      ...
      case 'autonomous': return 'text-yellow-400 bg-yellow-400/20 border-yellow-400/30'
      default: return 'text-gray-400 bg-gray-400/20 border-gray-400/30'
    }
  }

  // Safely parse fees/cutoff (DB stores them as JSON strings)
  const safeFees = (() => {
    const f = college.fees
    if (!f) return 'N/A'
    try { const p = JSON.parse(f); return typeof p === 'string' ? p : f } catch { return f }
  })()

  const safeCutoff = (() => {
    const c = college.cutoff
    if (!c) return 'N/A'
    try { const p = JSON.parse(c); return typeof p === 'string' ? p : c } catch { return c }
  })()

  const safeCourses: string[] = (() => {
    const c = college.courses
    if (!c) return []
    if (Array.isArray(c)) return c
    try { const p = JSON.parse(c as any); return Array.isArray(p) ? p : [] } catch { return [] }
  })()
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-4 mb-4">
        <div className="bg-black/20 p-3 rounded-lg">
          ...
          <p className="text-neon-cyan font-bold text-sm">{safeFees}</p>
        </div>
        ...
        <div className="bg-black/20 p-3 rounded-lg">
          ...
          <p className="text-neon-pink font-bold text-sm">{safeCutoff}</p>
        </div>
      </div>
